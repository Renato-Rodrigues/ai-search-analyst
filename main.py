...
def main():
    ...
    for query in root_queries:
        ...
        for key, value in out['result'].items(): # if more than two dimensionnal result
            io_service.set_value(value, out['output_sheet'], out['output_column'], value_type=out['output_type']) # Write the result to the sheet and clear if necessary
    ...
    for value in input_value:
        ...
        for query in level_1_queries:
            out = replace_placeholders(query, placeholders_variables) # Process the query, replacing placeholders in specific elements
            out['result'] = ai_query(out['query'], out['role'], out['format'], ai_service, model, disable_cache=out['disable_cache']) # Perform the AI query using the processed information
            io_service.set_value(out['query'], out['output_sheet'], out['output_column']) # Write the prompt to the Google Sheet
        ...
        for value2 in input_value2:
            ...
            for query in level_2_search_queries:
                out = replace_placeholders(query, placeholders_variables) # Process the query, replacing placeholders in specific elements
                out['num_results'] = min(config['test']['search_results'], int(out['num_results'])) if config['test_mode'] else int(out['num_results'])  # Number of results to retrieve, limiting to 10 if in test mode to avoid API quota limits
                ...
                io_service.set_value(value2, out['output_sheet'], out['output_column']) # Write the input 2 value to the sheet
                ...
                parent_row = out['output_row'] + 2
                for i, search_result in enumerate(output_dict[value][value2][out['title']]['result'][:min(config['test']['search_results'],config['test']['search_llm_queries']) if config['test_mode'] else None]): # Iterate through search results, limiting to the first item if in test mode to avoid API usage during test phase
                    ...
                    link_row = parent_row+1+i
                    level_3_queries = [q for q in llm_queries if q.get('level') == '3']
                    for query in level_3_queries:
                        out = replace_placeholders(query, placeholders_variables) # Process the query, replacing placeholders in specific elements
                        out['result'] = ai_query(out['query'], out['role'], out['format'], ai_service, model, disable_cache=out['disable_cache']) # Perform the AI query using the processed information
                        out['output_row'] = link_row #find_row_with_content(out['output_sheet'], 'C', search_result['link'])
                        if i==0:
                            io_service.set_value(list(out['result'].keys()), out['output_sheet'], out['output_column'], parent_row) # write headers
                        io_service.set_value(out['result'], out['output_sheet'], out['output_column'], link_row, out['output_type'], write_headers=False)
